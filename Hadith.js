...
const apiUrl =
  "https://www.hadithapi.com/api/hadiths?apiKey=$2y$10$zhgih4yx55xV2qkr2oVESel1V3aqStMNDmPPCzCz9MnFHlNqYnUPu&" +
  hadithNumberForApi;
fetch(apiUrl)
...
function convertMonth() {
  ...
  return todayEnglishDate;
}

function getArabicDate(todayEnglishDate) {
  ...
  const arabicDateApiUrl = "https://api.aladhan.com/v1/gToH/" + formattedToday;
  fetch(arabicDateApiUrl)
    ...
    .then((dateApi) => {
      ...
      document.getElementById("arabicDateNumber").innerHTML =
        dateApi.data.hijri.day;
      document.getElementById("arabicDateMonth").innerHTML =
        dateApi.data.hijri.month.en;
    })
    .catch((error) => {
      console.error(error);
    });
